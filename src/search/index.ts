  ...
  try {
    if (poolIds) {
      ...
      for (let i = 0; i < poolIds.length; i++) {
        let latestIndexTransaction = await getLatestPoolSearchIndexTxId(
          poolIds[i]
        );
        if (!latestIndexTransaction) continue;
        let latestIndexTransactionId = getTagValue(
          latestIndexTransaction.node.tags,
          TAGS.keys.uploaderTxId
        );
        let poolSearchState = (
          await getPoolSearchIndexById(latestIndexTransactionId)
        ).state;
        if (!poolSearchState || !poolSearchState.searchIndeces) {
          continue;
        }
        let thisPoolIndeces = poolSearchState.searchIndeces.map(
          (index: string) => {
...
async function searchIndex(
  ...
  let indeces = [...text.matchAll(new RegExp(searchTerm, "gi"))].map(
    (a) => a.index
  );
