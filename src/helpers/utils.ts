...
export function formatArtifactType(artifactType: string) {
  return artifactType.includes("Alex")
    ? artifactType.substring(5)
    : artifactType;
}
...
export function formatDate(
  ...
  return `${date.toLocaleString("default", {
    month: "long",
  })} ${date.getDate()}, ${date.getUTCFullYear()} @ ${getHours(
    date.getHours()
  )}:${formatTime(date.getMinutes())}:${formatTime(
    date.getSeconds()
...
export function checkAssociation(tags: KeyValueType[]) {
  return (
    getTagValue(tags, TAGS.keys.associationId) !== "" &&
    getTagValue(tags, TAGS.keys.associationId) !== STORAGE.none
  );
}

export async function traverseCommentTree(
