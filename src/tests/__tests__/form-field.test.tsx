...
test("Simulates initial input value", async () => {
    ...
    render(wrapWithProviders(
        <FormField
            ...
            disabled={false}
            invalid={{ status: false, message: null }}
            testingCtx={"test-form-field-init"}
        />
    ));
...
test("Simulates style", () => {
    ...
    render(wrapWithProviders(
        <FormField
            ...
            onChange={onChange}
            disabled={false}
            invalid={{ status: true, message: "Invalid Value" }}
            testingCtx={"test-form-field-styles"}
        />
    ));
    const input = screen.getByTestId("test-form-field-styles");
    expect(input).toHaveStyleRule(
        "border",
        `1px solid ${defaultTheme.colors.form.invalid.outline}`
    );
});
...
test("Simulates disabled", () => {
    ...
    render(wrapWithProviders(
        <FormField
            ...
            disabled={true}
            invalid={{ status: true, message: "Invalid Value" }}
            testingCtx={"test-form-field-disabled"}
        />
    ));
...
test("Simulates input change", () => {
    const onChange = jest.fn();
    const { rerender } = render(wrapWithProviders(
        <FormField
            value={""}
            onChange={onChange}
            disabled={false}
            invalid={{ status: false, message: null }}
            testingCtx={"test-form-field-input"}
        />
    ));
    const input = screen.getByTestId("test-form-field-input");
    fireEvent.change(input, { target: { value: "Test Value" } });
    rerender(wrapWithProviders(
        <FormField
            testingCtx={"test-form-field-input"}
            value={"Test Value"}
            onChange={onChange}
