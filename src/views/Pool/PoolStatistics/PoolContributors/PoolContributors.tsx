      ...
      for (let i = 0; i < sortedKeys.length; i++) {
        contributorList.push(
          <S.Row key={i} isEnd={i !== 2}>
            ...
            <S.Owner>
              <Link to={`${urls.libraryAll(sortedKeys[i])}`}>
                {formatAddress(sortedKeys[i], false)}
              </Link>
            </S.Owner>
            <S.Amount>
              <p>
                {arClient.getARAmount(
                  arClient.calcContributions(
                    props.data.state.contributors[sortedKeys[i]]
                  )
                )}
              </p>
              &nbsp;
          ...
          <S.Row key={i} isEnd={i !== 2}>
            ...
            <S.Amount>
              <p>
                {arClient.getARAmount(
                  arClient.calcContributions(
                    props.data.state.contributors[contributorKeys[i]]
                  )
                )}
              </p>
              &nbsp;
