...
export default function PoolStatistics(props: IProps) {
  ...
  async function getDetailData(contractSrc: string) {
    ...
    do {
      page++;
      const redstoneContracts = await fetch(
        getRedstoneDescEndpoint(contractSrc, page, 100)
      );

      const ids = (await redstoneContracts.json()).contracts.map(
        (contract: any) => {
      ...
      if (props.headerData) {
        ...
        if (dataContractSrc && dataContractSrc.contracts.length > 0) {
          setDetailData(
            await getDetailData(
              getTagValue(
                dataContractSrc.contracts[0].node.tags,
                TAGS.keys.contractSrc
              )
            )
          );
        } else {
