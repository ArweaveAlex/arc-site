...
export async function getArtifactsByAssociation(
  ...
  const range = Array.from(
    { length: sequence.end - sequence.start + 1 },
  ...
  if (associationId) {
    ...
    for (let i = 0; i < gqlArtifacts.length; i++) {
      const associationSequence = getTagValue(
        gqlArtifacts[i].node.tags,
        TAGS.keys.associationSequence
      );
...
export async function getArtifactById(
  artifactId: string
): Promise<ArtifactDetailType | null> {
...
export async function getArtifact(
  artifact: GQLResponseType
): Promise<ArtifactDetailType | null> {
  let pool: PoolType | null = await getPoolById(
    getTagValue(artifact.node.tags, TAGS.keys.poolId)
  );
    ...
    if (response.status === 200 && artifact) {
      try {
        return {
          ...
          artifactType: getTagValue(
            artifact.node.tags,
            TAGS.keys.artifactType
          ) as any,
          associationId: getTagValue(
            artifact.node.tags,
            TAGS.keys.associationId
          ),
          associationSequence: getTagValue(
            artifact.node.tags,
            TAGS.keys.associationSequence
          ),
          profileImagePath: getTagValue(
            artifact.node.tags,
            TAGS.keys.profileImage
          ),
...
export async function getArtifactsByPool(
  ...
  const artifacts: GQLResponseType[] = (
    ...
    return (
      getTagValue(element.node.tags, TAGS.keys.uploaderTxId) === STORAGE.none
    );
  });
...
export async function getArtifactsByIds(
  ...
  if (
    args.cursor !== CURSORS.p1 &&
    args.cursor !== CURSORS.end &&
    !checkGqlCursor(args.cursor)
  ) {
    cursor = args.cursor;
  }
...
export async function getBookmarkIds(owner: string): Promise<string[]> {
  ...
  if (bookmarks.length > 0) {
    let recentDate = Number(
      getTagValue(bookmarks[0].node.tags, TAGS.keys.dateCreated)!
    );

    for (let i = 0; i < bookmarks.length; i++) {
      const date = Number(
        getTagValue(bookmarks[i].node.tags, TAGS.keys.dateCreated)!
      );
    ...
    for (let i = 0; i < bookmarks.length; i++) {
      if (
        recentDate ===
        Number(getTagValue(bookmarks[i].node.tags, TAGS.keys.dateCreated)!)
      ) {
        return JSON.parse(
          getTagValue(bookmarks[i].node.tags, TAGS.keys.bookmarkIds)!
        );
      }
    }
...
export async function setBookmarkIds(
  owner: string,
  ids: string[]
): Promise<NotificationResponseType> {
  let txRes = await arClient.arweavePost.createTransaction(
