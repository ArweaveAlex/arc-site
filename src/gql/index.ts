...
export async function getGQLData(args: {
  ...
  if (
    args.reduxCursor &&
    args.cursorObject &&
    args.cursorObject === CursorEnum.Search
  ) {
    let i: number;
    let cursor: string;
    ...
    if (store.getState().searchIdsReducer[args.reduxCursor][i]) {
      ids = JSON.stringify(
        store.getState().searchIdsReducer[args.reduxCursor][i][cursor]
      );
      nextCursor =
        JSON.parse(ids).length < PAGINATOR
          ? CURSORS.end
          : `${SEARCH.cursorPrefix}-${++i}`;
    }
  }
...
function handleCursors(
  ...
  if (
    reduxCursor &&
    cursorObject &&
    store.getState().cursorsReducer[cursorObject][reduxCursor]
  ) {
  ...
  if (reduxCursor && cursorState) {
    ...
    if (nextCursor && cursorList[cursorList.length - 1] !== nextCursor)
