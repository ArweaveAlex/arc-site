          ...
          if (mediaIdsJsonKeys[i].indexOf(".")) {
            mediaComponents.push(
              <S.MediaElement key={mediaId}>
                {getMediaType(
                  mediaIdsJsonKeys[i].slice(
                    mediaIdsJsonKeys[i].indexOf(".") + 1
                  ),
...
function ChildAsset(props: { id: string }) {
  const [detailData, setDetailData] = React.useState<ArtifactDetailType | null>(
    null
  );
...
export default function MessagingListItem(props: IProps) {
  ...
  function getChildAssets() {
    if (
      props.data &&
      props.data.childAssets &&
      props.data.childAssets !== STORAGE.none
    ) {
  ...
  const artifactLink = props.data
    ? `${urls.artifact}${props.data.artifactId}`
    : "#";
      ...
      <S.LIContent>
        <S.LIHeader>
          <S.ProfileWrapper>
            ...
            <S.NUContainer>
              <S.Name>
                {jsonData.user && jsonData.user.name
                  ? jsonData.user.name
                  : STORAGE.none}
              </S.Name>
                ...
                <>
                  <S.ALink>
                    ...
                    <Link to={artifactLink}>
                      {props.data
                        ? formatAddress(props.data.artifactId, false)
                        : null}
                    </Link>
                  </S.ALink>
                  <S.ALinkNT>
        ...
        <S.LIBody>
          ...
          {jsonData.created_at && (
            <S.PostDate>{formatDate(jsonData.created_at, "iso")}</S.PostDate>
          )}
