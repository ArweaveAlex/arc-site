      ...
      <S.SAFormContainer onSubmit={() => props.handleSubmit(Number(amount))}>
        ...
        <S.SASubmit>
          <Button
            ...
            disabled={
              invalid || Number(amount) <= 0 || Number(amount) % 1 !== 0
            }
            formSubmit
            noMinWidth
          />
...
export default function StampWidget(props: IProps) {
  ...
  const [showWalletConnect, setShowWalletConnect] =
  ...
  const [stampCheckLoading, setStampCheckLoading] =
    React.useState<boolean>(false);
  const [stampNotification, setStampNotification] =
    React.useState<NotificationResponseType | null>(null);
  ...
  React.useEffect(() => {
    (async function () {
      if (props.txId) {
        setStampCheckLoading(true);
        const hasStamped = await stamps.hasStamped(
          props.walletAddress,
          props.txId
        );
        setStampCheckLoading(false);
  ...
  const handleStamp = React.useCallback(
    async (amount?: number) => {
      if (props.txId) {
        ...
        const stampSuccess =
          stamp && stamp.bundlrResponse && stamp.bundlrResponse.id;
        setStampCheckLoading(false);
            ...
            <S.WidgetContainer>
              <S.Action>
                <IconButton
                  ...
                  disabled={
                    stampDisabled || stampCheckLoading || !props.walletAddress
                  }
