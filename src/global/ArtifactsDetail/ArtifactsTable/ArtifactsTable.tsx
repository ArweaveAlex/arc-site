...
export default function ArtifactsTable(props: IProps) {
  const [data, setData] = React.useState<
    { data: ArtifactTableRowType; active: boolean; viewed: boolean }[] | null
  >(null);
  const [selectedCallbackIdsState, setSelectedCallbackIdsState] =
  ...
  function getArtifactLinkLabel(tags: KeyValueType[]) {
    return parse(
      formatMessagingText(getTagValue(tags, TAGS.keys.artifactName))
    );
  }
    ...
    if (props.showSearch) {
      return (
        <ArtifactsSearch
          ...
          setSearchRequested={(searchRequested: boolean) =>
        ...
        setData(
          props.data.contracts
            .map((element: any) => {
              ...
              if (props.showActions) {
                row.actions = getActionDropdown(
                  element.node.id,
                  element.node.tags
                );
              }
              if (
                getTagValue(element.node.tags, TAGS.keys.uploaderTxId) ===
                STORAGE.none
              ) {
                return {
                  data: row,
  ...
  React.useEffect(() => {
    (async function () {
      if (props.data && props.data.contracts.length > 0) {
        ...
        setStamps(
          await stamps.counts(
            props.data.contracts.map(
              (element: GQLResponseType) => element.node.id
            )
          )
        );
      }
