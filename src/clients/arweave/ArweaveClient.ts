...
export default class ArweaveClient {
  ...
  async getUserContributions(userWallet: string) {
    ...
    if (pools.length > 0) {
      const lastContributions: any = await this.calcLastContributions(
        userWallet,
        pools
      );
      return pools
        ...
        .map((pool: any) => {
          ...
          poolElement.receivingPercent = this.calcReceivingPercent(
            userWallet,
            pool
          );
          return poolElement;
  ...
  calcReceivingPercent(userWallet: string, pool: PoolType) {
    if (pool) {
      let calc =
        (parseFloat(
          this.calcContributions(pool.state.contributors[userWallet])
        ) /
  ...
  async calcLastContributions(userWallet: string, pools: PoolType[]) {
    ...
    for (let i = 0; i < pools.length; i++) {
      ...
      for (let j = 0; j < artifacts.contracts.length; j++) {
        const date = parseInt(
          getTagValue(artifacts.contracts[j].node.tags, TAGS.keys.dateCreated)
        );
        if (date > lastDate) {
          lastDate = date;
  ...
  getReceivingPercent(
    ...
    if (userWallet && contributors && totalContributions) {
      ...
      if (contributors[userWallet]) {
        amount = parseFloat(
          contributors[userWallet] + (!isNaN(activeAmount) ? activeAmount : 0)
        );
      }
  ...
  async handlePoolContribute(
    ...
    try {
      ...
      const { data: contractData }: { data: any } =
        await this.arweavePost.api.get(`/${fetchId}`);

      let owner = contractData.owner;
      if (arweaveContract) {
        owner = JSON.parse(
          Buffer.from(contractData.data, "base64").toString("utf-8")
        ).owner;
      }
      if (!owner) {
